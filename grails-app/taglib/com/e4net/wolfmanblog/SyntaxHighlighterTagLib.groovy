...
class SyntaxHighlighterTagLib {
	...
	static themePrefix = "shTheme"

	/**
	 * Loads the resources for each language. This overrides all the individual
	 *
	 * Example: <syntax:resources languages="['Java', 'Xml']" />
	 * Example: <syntax:resources languages="['Java', 'Xml']" gutter="true" firstLine="8" theme="Emacs"/>
	 *
	 * @param theme one of Default|Emacs|RDark|Django|Eclipse|FadeToGrey|Midnight
	 * @param languages List of languages. Currently supported names: see http://code.google.com/p/syntaxhighlighter/wiki/Languages
	 */
	def resources = { attrs ->
		if(!attrs.languages) {
			throwTagError("Tag resources is missing required attribute [languages]. Specify at least one language.")
		}

		def theme= "${themePrefix}Default.css"
		if(attrs.theme)
			theme= "${themeProfix}${attrs.theme}.css"

		// Add the stylesheet
		out << """<link type='text/css' rel='stylesheet' href='${resource(dir: 'css/syntaxhighlighter', file: 'shCore.css')}'/>
<link type='text/css' rel='stylesheet' href='${resource(dir: 'css/syntaxhighlighter', file: theme)}'/>\n"""
